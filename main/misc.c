...
void cut_pic(int on)
{
	char cmd[]={0x5a,0xa5,0x15,0x82,0x0b,0xa1,0x00,0x06,0x00,0x01,0x00,0x22,
				0x02,0x88,0x00,0xa6,0x03,0x52,0x00,0xf4,0x03,0x21,0x00,0x57};
	
	char off[]={0x5a,0xa5,0x15,0x82,0x0b,0xa1,0x00,0x06,0x00,0x01,0x00,0x01,
				0x03,0x21,0x00,0x57,0x03,0xe8,0x00,0x09b,0x03,0x21,0x00,0x57};
	if(on)
...
{
	char cmd1[]={0x5a,0xa5,0x15,0x82,0x0d,0xa9,0x00,0x06,0x00,0x01,0x00,0x01,
				0x03,0x53,0x00,0x0d,0x03,0x9b,0x00,0x4a,0x03,0x53,0x00,0x0d};
	char cmd2[]={0x5a,0xa5,0x15,0x82,0x0d,0xbb,0x00,0x06,0x00,0x01,0x00,0x02,
				0x03,0x53,0x00,0x0c,0x03,0x9b,0x00,0x49,0x03,0x53,0x00,0x0c};

	char off1[]={0x5a,0xa5,0x15,0x82,0x0d,0xa9,0x00,0x06,0x00,0x01,0x00,0x33,
				0x03,0x53,0x00,0x0d,0x03,0x9b,0x00,0x4a,0x03,0x53,0x00,0x0d};
	char off2[]={0x5a,0xa5,0x15,0x82,0x0d,0xbb,0x00,0x06,0x00,0x01,0x00,0x34,
				0x03,0x53,0x00,0x0c,0x03,0x9b,0x00,0x49,0x03,0x53,0x00,0x0c};
	if(on)
	{
...
void show_audio(int on)
{
	char cmd1[]={0x5a,0xa5,0x15,0x82,0x0d,0xb2,0x00,0x06,0x00,0x01,0x00,0x01,
				0x03,0xa0,0x00,0x0f,0x03,0xe8,0x00,0x4c,0x03,0xa0,0x00,0x0f};
	char cmd2[]={0x5a,0xa5,0x15,0x82,0x0d,0xc4,0x00,0x06,0x00,0x01,0x00,0x02,
				0x03,0xa0,0x00,0x0c,0x03,0xe8,0x00,0x49,0x03,0xa0,0x00,0x0c};

	char off1[]={0x5a,0xa5,0x15,0x82,0x0d,0xb2,0x00,0x06,0x00,0x01,0x00,0x33,
				0x03,0xa0,0x00,0x0f,0x03,0xe8,0x00,0x4c,0x03,0xa0,0x00,0x0f};
	char off2[]={0x5a,0xa5,0x15,0x82,0x0d,0xc4,0x00,0x06,0x00,0x01,0x00,0x34,
				0x03,0xa0,0x00,0x0c,0x03,0xe8,0x00,0x49,0x03,0xa0,0x00,0x0c};
	if(on)
	{
		write(g_share_memory->fd_lcd,cmd1,sizeof(cmd1));
...
void gprs_state(int state,int pic)
{
	char good[]={0x5a,0xa5,0x15,0x82,0x06,0xf3,0x00,0x06,0x00,0x01,0x00,0x22,
				0x01,0xae,0x01,0x76,0x02,0x75,0x01,0xc3,0x01,0xa8,0x01,0x6f};
	
	char bad[]={0x5a,0xa5,0x15,0x82,0x06,0xf3,0x00,0x06,0x00,0x01,0x00,0x22,
				0x02,0x87,0x01,0x76,0x03,0x4e,0x01,0xc3,0x02,0x81,0x01,0x6f};
	...
	else//gprs bad
	{
		...
		bad[12]=0x02;bad[13]=0x80;
		bad[14]=0x00;bad[15]=0xa6;
		bad[16]=0x03;bad[17]=0x4f;
		bad[18]=0x00;bad[19]=0xf7;
	}
...
int beyond_alarm(char *alarm,char *data)
{
	...
	i=0;j=0;
	char *out2=(char *)malloc(strlen(data));
	memset(out2,'\0',strlen(data));
	while(data[i]!='\0')
	{
		if(data[i]!='.')
			out2[j++]=data[i];
	...
	if(atoi(out1)>atoi(out2))
	{
		free(out1);free(out2);
		return 1;
	}
	else if(atoi(out1)<atoi(out2))
	{
		free(out1);free(out2);
		return 0;
	}
	else
	{
		...
		else
		{
			...
			{
				int a=atoi(out2);
				for(i=0;i<strlen(out1)-strlen(out2);i++)
					a=a*10;
				if(atoi(out1)>=a)
					result=1;
			}
...
void show_main_alarm_co(char *data)
{
	...
	else 
	{
			...
			data))
		{
			...
			info.x00=(100>>8)&0xff;info.y00=(342>>8)&0xff;
			info.x01=100&0xff;info.y01=342&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((342+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(342+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;			
			info.x00=(100>>8)&0xff;info.y00=(344>>8)&0xff;
			info.x01=100&0xff;info.y01=344&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((344+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(344+18)&0xff;
...
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_CO2_SHOW1,0x0001);
	write_data(ADDR_ALARM_CO2_SHOW2,0x0001);
}
else 
{
	write_data(ADDR_ALARM_CO2_SHOW1,0x0000);
	write_data(ADDR_ALARM_CO2_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.co2,data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_CO2_1>>8)&0xff;
		info.vp1=ADDR_CUT_CO2_1&0xff;
		info.x00=(100>>8)&0xff;info.y00=(406>>8)&0xff;
		info.x01=100&0xff;info.y01=406&0xff;
		info.x10=((100+19)>>8)&0xff;info.y10=((406+18)>>8)&0xff;
		info.x11=(100+19)&0xff;info.y11=(406+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(100>>8)&0xff;info.y00=(409>>8)&0xff;
		info.x01=100&0xff;info.y01=409&0xff;
		info.x10=((100+19)>>8)&0xff;info.y10=((409+18)>>8)&0xff;
		info.x11=(100+19)&0xff;info.y11=(409+18)&0xff;
		info.vp0=(ADDR_CUT_CO2_2>>8)&0xff;
		info.vp1=ADDR_CUT_CO2_2&0xff;
		cut(info);
	}
	else
	{
...
void show_main_alarm_hcho(char *data)
{
	...
	{
		/* no alarm point , no display */
		write_data(ADDR_ALARM_HCHO_SHOW1,0x0001);
		write_data(ADDR_ALARM_HCHO_SHOW2,0x0001);
	}
	else 
	{
		write_data(ADDR_ALARM_HCHO_SHOW1,0x0000);
		write_data(ADDR_ALARM_HCHO_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.hcho,data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
			info.vp0=(ADDR_CUT_HCHO_1>>8)&0xff;
			info.vp1=ADDR_CUT_HCHO_1&0xff;
			info.x00=(100>>8)&0xff;info.y00=(155>>8)&0xff;
			info.x01=100&0xff;info.y01=155&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((155+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(155+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			...
			info.x00=(100>>8)&0xff;info.y00=(154>>8)&0xff;
			info.x01=100&0xff;info.y01=154&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((154+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(154+18)&0xff;
			cut(info);
		}
		else
		{
...
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_TEMP_SHOW1,0x0001);
	write_data(ADDR_ALARM_TEMP_SHOW2,0x0001);
}
else 
{
	write_data(ADDR_ALARM_TEMP_SHOW1,0x0000);
	write_data(ADDR_ALARM_TEMP_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.temp,data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_TEMP_1>>8)&0xff;
		info.vp1=ADDR_CUT_TEMP_1&0xff;
		info.x00=(100>>8)&0xff;info.y00=(471>>8)&0xff;
		info.x01=100&0xff;info.y01=471&0xff;
		info.x10=((100+19)>>8)&0xff;info.y10=((471+18)>>8)&0xff;
		info.x11=(100+19)&0xff;info.y11=(471+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.page0=(MAIN_PAGE_PPM>>8)&0xff;
		...
		info.x00=(100>>8)&0xff;info.y00=(473>>8)&0xff;
		info.x01=100&0xff;info.y01=473&0xff;
		info.x10=((100+19)>>8)&0xff;info.y10=((473+18)>>8)&0xff;
		info.x11=(100+19)&0xff;info.y11=(473+18)&0xff;
		cut(info);
	}
	else
	{
...
void show_main_alarm_shidu(char *data)
{
	...
	{
		/* no alarm point , no display */
		write_data(ADDR_ALARM_SHIDU_SHOW1,0x0001);
		write_data(ADDR_ALARM_SHIDU_SHOW2,0x0001);
	}
	else 
	{
		write_data(ADDR_ALARM_SHIDU_SHOW1,0x0000);
		write_data(ADDR_ALARM_SHIDU_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.shidu,data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
			info.vp0=(ADDR_CUT_SHIDU_1>>8)&0xff;
			info.vp1=ADDR_CUT_SHIDU_1&0xff;
			info.x00=(514>>8)&0xff;info.y00=(156>>8)&0xff;
			info.x01=514&0xff;info.y01=156&0xff;
			info.x10=((514+19)>>8)&0xff;info.y10=((156+18)>>8)&0xff;
			info.x11=(514+19)&0xff;info.y11=(156+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			...
			info.x00=(514>>8)&0xff;info.y00=(154>>8)&0xff;
			info.x01=514&0xff;info.y01=154&0xff;
			info.x10=((514+19)>>8)&0xff;info.y10=((154+18)>>8)&0xff;
			info.x11=(514+19)&0xff;info.y11=(154+18)&0xff;
			cut(info);
		}
		else
		{
...
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_O3_SHOW1,0x0001);
	write_data(ADDR_ALARM_O3_SHOW2,0x0001);
}
else 
{
	write_data(ADDR_ALARM_O3_SHOW1,0x0000);
	write_data(ADDR_ALARM_O3_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.o3,data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_O3_1>>8)&0xff;
		info.vp1=ADDR_CUT_O3_1&0xff;
		info.x00=(514>>8)&0xff;info.y00=(471>>8)&0xff;
		info.x01=514&0xff;info.y01=471&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((471+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(471+18)&0xff;
		cut(info);
		info.page0=(MAIN_PAGE_PPM>>8)&0xff;
		info.page1=MAIN_PAGE_PPM&0xff;
		info.vp0=(ADDR_CUT_O3_2>>8)&0xff;
		info.vp1=ADDR_CUT_O3_2&0xff;
		info.x00=(514>>8)&0xff;info.y00=(475>>8)&0xff;
		info.x01=514&0xff;info.y01=475&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((475+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(475+18)&0xff;
		cut(info);
	}
	else
	{
...
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_TVOC_SHOW1,0x0001);
	write_data(ADDR_ALARM_TVOC_SHOW2,0x0001);
}
else 
{
	write_data(ADDR_ALARM_TVOC_SHOW1,0x0000);
	write_data(ADDR_ALARM_TVOC_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.tvoc,data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_TVOC_1>>8)&0xff;
		info.vp1=ADDR_CUT_TVOC_1&0xff;
		info.x00=(514>>8)&0xff;info.y00=(405>>8)&0xff;
		info.x01=514&0xff;info.y01=405&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((405+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(405+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(514>>8)&0xff;info.y00=(408>>8)&0xff;
		info.x01=514&0xff;info.y01=408&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((408+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(408+18)&0xff;
		info.vp0=(ADDR_CUT_TVOC_2>>8)&0xff;
		info.vp1=ADDR_CUT_TVOC_2&0xff;
		cut(info);
	}
	else
	{
...
void show_main_alarm_noise(char *data)
{
	...
	{
		/* no alarm point , no display */
		write_data(ADDR_ALARM_NOISE_SHOW1,0x0001);
		write_data(ADDR_ALARM_NOISE_SHOW2,0x0001);
	}
	else 
	{
		...
		if (beyond_alarm(g_share_memory->sensor_alarm_val.noise,data))
		{
			...
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
			info.vp0=(ADDR_CUT_NOISE_1>>8)&0xff;
			info.vp1=ADDR_CUT_NOISE_1&0xff;
			info.x00=(515>>8)&0xff;info.y00=(280>>8)&0xff;
			info.x01=515&0xff;info.y01=280&0xff;
			info.x10=((515+19)>>8)&0xff;info.y10=((280+18)>>8)&0xff;
			info.x11=(515+19)&0xff;info.y11=(280+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
			info.x00=(515>>8)&0xff;info.y00=(283>>8)&0xff;
			info.x01=515&0xff;info.y01=283&0xff;
			info.x10=((515+19)>>8)&0xff;info.y10=((283+18)>>8)&0xff;
			info.x11=(515+19)&0xff;info.y11=(283+18)&0xff;
			info.vp0=(ADDR_CUT_NOISE_2>>8)&0xff;
			info.vp1=ADDR_CUT_NOISE_2&0xff;
			cut(info);
		}
		else
		{
...
void show_main_alarm_pm25(char *data)
{
	...
	else 
	{
		...
		if (beyond_alarm(g_share_memory->sensor_alarm_val.pm25,data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
			...
			info.x00=(100>>8)&0xff;info.y00=(280>>8)&0xff;
			info.x01=100&0xff;info.y01=280&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((280+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(280+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;			
			info.x00=(100>>8)&0xff;info.y00=(282>>8)&0xff;
			info.x01=100&0xff;info.y01=282&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((282+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(282+18)&0xff;
...
void show_main_alarm_pm10(char *data)
{
	...
	{
		/* no alarm point , no display */
		write_data(ADDR_ALARM_PM10_SHOW1,0x0001);
		write_data(ADDR_ALARM_PM10_SHOW2,0x0001);
	}
	else 
	{
		write_data(ADDR_ALARM_PM10_SHOW1,0x0000);
		write_data(ADDR_ALARM_PM10_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.pm10,data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
			info.vp0=(ADDR_CUT_PM10_1>>8)&0xff;
			info.vp1=ADDR_CUT_PM10_1&0xff;
			info.x00=(100>>8)&0xff;info.y00=(217>>8)&0xff;
			info.x01=100&0xff;info.y01=217&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((217+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(217+18)&0xff;
			cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;			
			info.x00=(100>>8)&0xff;info.y00=(216>>8)&0xff;
			info.x01=100&0xff;info.y01=216&0xff;
			info.x10=((100+19)>>8)&0xff;info.y10=((216+18)>>8)&0xff;
			info.x11=(100+19)&0xff;info.y11=(216+18)&0xff;
			info.vp0=(ADDR_CUT_PM10_2>>8)&0xff;
			info.vp1=ADDR_CUT_PM10_2&0xff;
			cut(info);
		}
		else
		{
...
void init_alarm_show()
{
	...
	write_data(ADDR_ALARM_PM10_SHOW1,0x0001);
	write_data(ADDR_ALARM_PM10_SHOW2,0x0001);
	write_data(ADDR_ALARM_WIND_SHOW1,0x0001);
	write_data(ADDR_ALARM_WIND_SHOW2,0x0001);
	write_data(ADDR_ALARM_NOISE_SHOW1,0x0001);
	write_data(ADDR_ALARM_NOISE_SHOW2,0x0001);
	write_data(ADDR_ALARM_TVOC_SHOW1,0x0001);
	write_data(ADDR_ALARM_TVOC_SHOW2,0x0001);
	...
	write_data(ADDR_ALARM_CO2_SHOW1,0x0001);
	write_data(ADDR_ALARM_CO2_SHOW2,0x0001);
	write_data(ADDR_ALARM_PRESS_SHOW1,0x0001);
	write_data(ADDR_ALARM_PRESS_SHOW2,0x0001);
}
void show_main_alarm()
{
	...
	else 
	{
		write_data(ADDR_ALARM_HCHO_SHOW1,0x0000);
		write_data(ADDR_ALARM_HCHO_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.hcho,
			sensor_history.hcho[g_share_memory->cnt[SENSOR_HCHO]-1].data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
	...
	else 
	{
		write_data(ADDR_ALARM_SHIDU_SHOW1,0x0000);
		write_data(ADDR_ALARM_SHIDU_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.shidu,
			sensor_history.shidu[g_share_memory->cnt[SENSOR_SHIDU]-1].data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
	...
	else 
	{
		write_data(ADDR_ALARM_PM10_SHOW1,0x0000);
		write_data(ADDR_ALARM_PM10_SHOW2,0x0000);
		if (beyond_alarm(g_share_memory->sensor_alarm_val.pm10,
			sensor_history.pm10[g_share_memory->cnt[SENSOR_PM10]-1].data))
		{
			/* <, show green */
			info.page0=(MAIN_PAGE>>8)&0xff;
			info.page1=MAIN_PAGE&0xff;
...
/*1 get history data <> alarm point */
if (strlen(g_share_memory->sensor_alarm_val.wind) ==0)
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_WIND_SHOW1,0x0001);
	write_data(ADDR_ALARM_WIND_SHOW2,0x0001);
...
{
	write_data(ADDR_ALARM_WIND_SHOW1,0x0000);
	write_data(ADDR_ALARM_WIND_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.wind,
		sensor_history.wind[g_share_memory->cnt[SENSOR_WIND]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_WIND_1>>8)&0xff;
		info.vp1=ADDR_CUT_WIND_1&0xff;
		info.x00=(515>>8)&0xff;info.y00=(218>>8)&0xff;
		info.x01=515&0xff;info.y01=218&0xff;
		info.x10=((515+19)>>8)&0xff;info.y10=((218+18)>>8)&0xff;
		info.x11=(515+19)&0xff;info.y11=(218+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(515>>8)&0xff;info.y00=(216>>8)&0xff;
		info.x01=515&0xff;info.y01=216&0xff;
		info.x10=((515+19)>>8)&0xff;info.y10=((216+18)>>8)&0xff;
		info.x11=(515+19)&0xff;info.y11=(216+18)&0xff;
		info.vp0=(ADDR_CUT_WIND_2>>8)&0xff;
		info.vp1=ADDR_CUT_WIND_2&0xff;
		cut(info);
	}
	else
	{
...
else 
{
		...
		sensor_history.noise[g_share_memory->cnt[SENSOR_NOISE]-1].data))
	{
		...
		info.vp0=(ADDR_CUT_NOISE_1>>8)&0xff;
		info.vp1=ADDR_CUT_NOISE_1&0xff;
		info.x00=(513>>8)&0xff;info.y00=(280>>8)&0xff;
		info.x01=513&0xff;info.y01=280&0xff;
		info.x10=((513+19)>>8)&0xff;info.y10=((280+18)>>8)&0xff;
		info.x11=(513+19)&0xff;info.y11=(280+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(513>>8)&0xff;info.y00=(283>>8)&0xff;
		info.x01=513&0xff;info.y01=283&0xff;
		info.x10=((513+19)>>8)&0xff;info.y10=((283+18)>>8)&0xff;
		info.x11=(513+19)&0xff;info.y11=(283+18)&0xff;
		info.vp0=(ADDR_CUT_NOISE_2>>8)&0xff;
		info.vp1=ADDR_CUT_NOISE_2&0xff;
		cut(info);
	}
...
/*1 get history data <> alarm point */
if (strlen(g_share_memory->sensor_alarm_val.press) ==0)
{
	/* no alarm point , no display */
	write_data(ADDR_ALARM_PRESS_SHOW1,0x0001);
	write_data(ADDR_ALARM_PRESS_SHOW2,0x0001);
...
{
	write_data(ADDR_ALARM_PRESS_SHOW1,0x0000);
	write_data(ADDR_ALARM_PRESS_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.press,
		sensor_history.press[g_share_memory->cnt[SENSOR_PRESS]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_PRESS_1>>8)&0xff;
		info.vp1=ADDR_CUT_PRESS_1&0xff;
		info.x00=(514>>8)&0xff;info.y00=(342>>8)&0xff;
		info.x01=514&0xff;info.y01=342&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((342+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(342+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(514>>8)&0xff;info.y00=(344>>8)&0xff;
		info.x01=514&0xff;info.y01=344&0xff;
		info.x10=((514+19)>>8)&0xff;info.y10=((344+18)>>8)&0xff;
		info.x11=(514+19)&0xff;info.y11=(344+18)&0xff;
		info.vp0=(ADDR_CUT_PRESS_2>>8)&0xff;
		info.vp1=ADDR_CUT_PRESS_2&0xff;
		cut(info);
	}
	else
	{
...
else 
{
	write_data(ADDR_ALARM_CO2_SHOW1,0x0000);
	write_data(ADDR_ALARM_CO2_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.co2,
		sensor_history.co2[g_share_memory->cnt[SENSOR_CO2]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		...
		info.vp0=(ADDR_CUT_CO2_1>>8)&0xff;
		info.vp1=ADDR_CUT_CO2_1&0xff;
		info.x00=(101>>8)&0xff;info.y00=(406>>8)&0xff;
		info.x01=101&0xff;info.y01=406&0xff;
		info.x10=((101+19)>>8)&0xff;info.y10=((406+18)>>8)&0xff;
		info.x11=(101+19)&0xff;info.y11=(406+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(101>>8)&0xff;info.y00=(409>>8)&0xff;
		info.x01=101&0xff;info.y01=409&0xff;
		info.x10=((101+19)>>8)&0xff;info.y10=((409+18)>>8)&0xff;
		info.x11=(101+19)&0xff;info.y11=(409+18)&0xff;
		info.vp0=(ADDR_CUT_CO2_2>>8)&0xff;
		info.vp1=ADDR_CUT_CO2_2&0xff;
		cut(info);
	}
...
else 
{
	write_data(ADDR_ALARM_TVOC_SHOW1,0x0000);
	write_data(ADDR_ALARM_TVOC_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.tvoc,
		sensor_history.tvoc[g_share_memory->cnt[SENSOR_TVOC]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_TVOC_1>>8)&0xff;
		info.vp1=ADDR_CUT_TVOC_1&0xff;
		info.x00=(513>>8)&0xff;info.y00=(405>>8)&0xff;
		info.x01=513&0xff;info.y01=405&0xff;
		info.x10=((513+19)>>8)&0xff;info.y10=((405+18)>>8)&0xff;
		info.x11=(513+19)&0xff;info.y11=(405+18)&0xff;
		cut(info);
			info.page0=(MAIN_PAGE_PPM>>8)&0xff;
			info.page1=MAIN_PAGE_PPM&0xff;
		info.x00=(513>>8)&0xff;info.y00=(408>>8)&0xff;
		info.x01=513&0xff;info.y01=408&0xff;
		info.x10=((513+19)>>8)&0xff;info.y10=((408+18)>>8)&0xff;
		info.x11=(513+19)&0xff;info.y11=(408+18)&0xff;
		info.vp0=(ADDR_CUT_TVOC_2>>8)&0xff;
		info.vp1=ADDR_CUT_TVOC_2&0xff;
		cut(info);
	}
...
else 
{
	write_data(ADDR_ALARM_TEMP_SHOW1,0x0000);
	write_data(ADDR_ALARM_TEMP_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.temp,
		sensor_history.temp[g_share_memory->cnt[SENSOR_TEMP]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
		info.vp0=(ADDR_CUT_TEMP_1>>8)&0xff;
		info.vp1=ADDR_CUT_TEMP_1&0xff;
		info.x00=(101>>8)&0xff;info.y00=(471>>8)&0xff;
		info.x01=101&0xff;info.y01=471&0xff;
		info.x10=((101+19)>>8)&0xff;info.y10=((471+18)>>8)&0xff;
		info.x11=(101+19)&0xff;info.y11=(471+18)&0xff;
		cut(info);
		info.page0=(MAIN_PAGE_PPM>>8)&0xff;
		info.page1=MAIN_PAGE_PPM&0xff;
		info.vp0=(ADDR_CUT_TEMP_2>>8)&0xff;
		info.vp1=ADDR_CUT_TEMP_2&0xff;
		info.x00=(101>>8)&0xff;info.y00=(473>>8)&0xff;
		info.x01=101&0xff;info.y01=473&0xff;
		info.x10=((101+19)>>8)&0xff;info.y10=((473+18)>>8)&0xff;
		info.x11=(101+19)&0xff;info.y11=(473+18)&0xff;
		cut(info);
	}
	else
	{
...
else 
{
	write_data(ADDR_ALARM_O3_SHOW1,0x0000);
	write_data(ADDR_ALARM_O3_SHOW2,0x0000);
	if (beyond_alarm(g_share_memory->sensor_alarm_val.o3,
		sensor_history.o3[g_share_memory->cnt[SENSOR_O3]-1].data))
	{
		/* <, show green */
		info.page0=(MAIN_PAGE>>8)&0xff;
		info.page1=MAIN_PAGE&0xff;
